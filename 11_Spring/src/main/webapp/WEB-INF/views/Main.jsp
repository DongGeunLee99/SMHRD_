					...
					<header id="header" class="alt">
						...
						<nav>
							...
							<c:if test="${not empty info}">
								<c:if test="${'admin' eq info.email }">
									<a href="showMember">회원정보관리</a>
								</c:if>
								<c:if test="${'admin' ne info.email }">
						...
						<!-- Q1. 회원가입 기능 만들기 -->
							<li><h5>회원가입</h5></li>
								<form action="join" method="post">
									...
									<li><input type="button" value="Email 중복체크" onclick="checkEmail()"></input></li>
				...
				function checkEmail(){
					...
					$.ajax({
						...
						success : function(result){
							...
							if(result == true){
								$('#resultEmail').text('사용할 수 있는 이메일')
							} else{
								$('#resultEmail').text('사용할 수 없는 이메일')
							}
